...
def register(cli):
    ...
    @click.option(
        ...
        help='Path to telemetry.db file (overrides config)'
    )
    @click.option(
    ...
    @click.option(
        ...
        help='Remote server URL to POST traces (overrides config)'
    )
    @click.option(
        '--auth-token', envvar='CLI_TELEMETRY_UPLOAD_TOKEN', default=None,
        help='Bearer token for authentication (overrides config)'
    )
    @click.option(
